...
resource "azurerm_role_assignment" "frontend_rbac" {
  ...
  scope                = each.value.vm_id
  role_definition_name = each.value.user.role_type == "admin" ? "Virtual Machine Administrator Login" : "Virtual Machine User Login"
  principal_id         = each.value.user.is_user == true ? data.azuread_user.frontend_users[each.value.user_key].id : each.value.user.is_group == true ? data.azuread_group.frontend_groups[each.value.user_key].id : data.azuread_service_principal.frontend_sps[each.value.user_key].id
}
