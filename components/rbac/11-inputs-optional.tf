...
variable "frontend_users" {
  ...
  description = "Map of objects describing the users, groups and service principals who should be able to access the Frontend VMs."
  default     = {}
  validation {
    condition = alltrue([
      for user in var.frontend_users : anytrue([user.is_group != null, user.is_user != null, user.is_service_principal != null])
      && contains(["admin", "user"], lower(user.role_type))
    ])
    error_message = "One of is_group, is_user or is_service_principal must be set to true for each user, group or service principal. The valid values for role_type are admin and user."
  }
